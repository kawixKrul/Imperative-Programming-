...
int cmp_date(const void *d1, const void *d2) {
    Date *test=(Date*)d1 , *test2 = (Date*)d2;
    if(test->day==test2->day && test->month==test2->month && test->year==test2->year){
        return 0;
    } else if(test->day>=test2->day && test->month>=test2->month && test->year>=test2->year){
        return 1;
    }
    else{
        return -1;
...
int cmp(const void *a, const void *b){
    Food *test=(Food*)a , *test2 = (Food*)b;
    if (test->price - test2->price == 0){
        return cmp_date(&test->valid_date,&test2->valid_date);
    }
    else if (test->price > test2->price){
        return 1 ;
    }
    else if (test->price < test2->price){
        return -1 ;
...
void *bsearch2(const void *key, void *base, size_t nitems, size_t size, ComparFp compar, char *result) {
    int firstRes=0;
    Food* start=(Food*)base;
    if (nitems==0){
        *result=0;
        return base;
    }
    for(int i=0; i < nitems;i++){
        firstRes=compar((Food*)key,&start[i]);
        if(firstRes==-1){
            *result=0;
            return (void*)&start[i];
        }
    ...
    *result=0;
    return (void*)&start[nitems];
}

void print_art(Food *p, size_t n, char *art) {
    for (int i=0; i<n; ++i) {
        if (strcmp(p->art, art)) {
            printf("%s %f %f %d.%d.%d", p->art, p->price, p->amount,
                   p->valid_date.day, p->valid_date.month, p->valid_date.year);
        }
    }
}
...
Food* add_record(Food *tab, size_t tab_size, int *np, ComparFp compar, Food *new) {
    ...
    Food* addr;
    int counter=0;
    addr = (Food*)bsearch2(new,tab,*np,*np,compar,&result);
    if(*np==0){
        (*np)++;
        *addr=*new;
    }else if(addr==&tab[*np]){
...
int read_stream(Food *tab, size_t size, int no, FILE *stream) {
    ...
    while(no) {
        Food new_f;
        int date[3];
        fscanf(stream,"%s %f %f %d.%d.%d", new_f.art, &new_f.price, &new_f.amount, &date[0], &date[1], &date[2]);
        Date new_d = {date[0], date[1], date[2]};
        new_f.valid_date = new_d;
        Food *added = add_record(tab, size, &np, cmp, &new_f);
        no -= 1;
    }
    return np;
}
...
int read_stream0(Food *tab, size_t size, int no, FILE *stream) {
    ...
    int day,month,year;
    for(int i=0; i < no;i++) {
        fscanf(stream, "%s %f %f %d %d %d", art, &price, &amount, &day, &month, &year);
        Food f = {};
        strcpy(f.art, art);
        f.price = price;
        f.amount = amount;
        ...
        f.valid_date = d;
        add_record(tab, FOOD_MAX, &ileaktualnie, cmp, &f);
