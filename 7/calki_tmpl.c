...
double quad_trap(Func1vFp func, double a, double b, int n) {  // Trapezow
    ...
    double sum = 0;
    for (int i = 0; i < n; ++i) {
        double Qt = (step/2) * (func(a) + func(a+step));
        a += step;
        sum += Qt;
    }
    return sum;
...
double quad_simpson(Func1vFp f, double a, double b, int n) {  // Simpsona
    ...
    for (int i = 0; i < n; ++i) {
        double c = (2*a+step)/2;
        double Qs = (step/6)*(f(a)+4*f(c)+f(a+step));
        a += step;
        sum += Qs;
    }
    return sum;
...
double dbl_integr_normal_1(Func2vFp f, double x1, double x2, int nx, double hy, Func1vFp fg, Func1vFp fh)  {
    ...
    for (int i = 0; i < nx; ++i) {
        double x = x1 + (i + 0.5) * (x2 - x1) / nx;
        int ny = (int)ceil((fh(x) - fg(x)) / hy);
        double yi = (fh(x) - fg(x)) / ny;
        for (int j = 0; j < ny; ++j) {
            double y = fg(x) + (j + 0.5) * yi;
            double fx = f(x, y);
            sum += fx * yi;
        }
    }
...
void recur_quad_rect_mid(double *psum, FuncNvFp f, int variable_no, double tvariable[], const double variable_lim[][2], const int tn[], int level, BoundNvFp boundary) {
    ...
    double a = variable_lim[level][0];
    double b = variable_lim[level][1];
    double h = (b - a) / tn[level];
    ...
    for (int i = 0; i < tn[level]; i++) {
        ...
        if (level == variable_no - 1) {
            double fx = f(vars, variable_no);
            if (!boundary || boundary(vars, variable_no))
            {
                *psum += fx/df;
            }
        }
