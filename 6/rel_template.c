...
int is_asymmetric(const pair* relation, int n) {
    return (is_antisymmetric(relation, n) == 1 && is_irreflexive(relation, n) == 1) ? 1 : 0;
}
...
int is_partial_order(const pair* relation, int n) {
    return (is_reflexive(relation, n) == 1 &&
            is_transitive(relation, n) == 1 &&
            is_antisymmetric(relation, n) == 1) ? 1 : 0;
}
...
int is_total_order(const pair* relation, int n) {
    return is_partial_order(relation, n) && is_connected(relation, n) ? 1 : 0;
}
