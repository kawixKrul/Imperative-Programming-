...
void set_null(size_t n, ht_element *ht) {
    for (int i = 0; i < n; ++i) {
        ht[i].next = NULL;
    }
}
...
void init_ht(hash_table *p_table, size_t size, DataFp dump_data, CreateDataFp create_data,
    ...
    set_null(p_table->size, p_table->ht);
...
void rehash(hash_table *p_table){
    p_table->size *= 2;
    ht_element *new = (ht_element*)malloc(sizeof(ht_element)*p_table->size);
    set_null(p_table->size, new);
    for (int i = 0; i < p_table->size/2; ++i) {
        ht_element *relocate = p_table->ht[i].next;
        while (relocate != NULL) {
            ht_element *temp = relocate;
            relocate = relocate->next;
            int new_hash = p_table->hash_function(temp->data, p_table->size);
            temp->next = new[new_hash].next;
            new[new_hash].next = temp;
        }
...
ht_element *find_previous(hash_table *p_table, data_union data) {
    ...
    ht_element *temp = &p_table->ht[hash];
    while (temp->next != NULL) {
        if (p_table->compare_data(temp->next->data, data) == 0) return temp;
        else temp = temp->next;
    }
    return NULL;
}
...
ht_element *get_element(hash_table *p_table, data_union *data) {
    ht_element *found = find_previous(p_table, *data);
...
void insert_element(hash_table *p_table, data_union *data) {
    ...
    if (el == NULL) {
        ht_element *insert = (ht_element *)malloc(sizeof(ht_element));
        insert->next = p_table->ht[hash].next;
        insert->data = *data;
        p_table->ht[hash].next = insert;
        p_table->no_elements += 1;
    } else {
        if (p_table->modify_data != NULL) {
            p_table->modify_data(&el->data);
        }
    }
    if ((double)p_table->no_elements / (double)p_table->size > (double )MAX_RATE) {
        rehash(p_table);
    }
}
...
void remove_element(hash_table *p_table, data_union data) {
    ...
    ht_element *del = get_element(p_table, &data);
    if (prev != NULL && del != NULL) {
        prev->next = del->next;
    }
    free_element(p_table->free_data,del);
}
...
void dump_word(data_union data) {
    printf("%s %d", ((DataWord*)data.ptr_data)->word, ((DataWord*)data.ptr_data)->counter*2);
}
...
int cmp_word(data_union a, data_union b) {
    char *aa = ((DataWord*)a.ptr_data)->word;
    char *bb = ((DataWord*)b.ptr_data)->word;
    return strcmp(aa, bb);
}
...
data_union create_data_word(void *value) {
    ...
    if (value == NULL) {
        ...
        strncpy(dat, (char*)value, 30);
    }
    DataWord *da_word = (DataWord*)malloc(sizeof(DataWord));
    da_word->word = malloc(sizeof(char)*30);
    strncpy(da_word->word, dat, 30);
    data_union new;
    new.ptr_data = da_word;
    return new;
}
...
void stream_to_ht(hash_table *p_table, FILE *stream) {
    ...
    while(fgets(buf,BUFFER_SIZE,stream)) {
        ...
        while(p) {
            data_union new = create_data_word(p);
...
int main(void) {
	...
	switch (to_do) {
		...
		case 3: // read words from text, insert into hash table, and print
			...
			//FILE *stream = fopen("Tekst_C.txt", "r"); //TO REMOVE
            stream_to_ht(&table, stdin);
			printf ("%zu\n", table.size);
            //data_union test = create_data_word("operating"); // TO REMOVE
            //dump_list(&table, hash_word(test, table.size)); //TO REMOVE
