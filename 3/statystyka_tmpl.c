...
void bernoulli_gen(int tab[], size_t n, double probab) {
    unsigned int p = probab*RAND_MAX+1;
    for (int i = 0; i < n; ++i) {
...
void two_dice_probab_distrib(double distrib[], int throws_num) {
    for (int i = 0; i < throws_num; ++i) {
        int kos1 = rand_from_interval(1,6);
        int kos2 = rand_from_interval(1,6);
        int sum = kos1 + kos2;
        distrib[sum-2] += 1;
    }
    for (int i = 0; i < 11; ++i) {
...
void print_histogram(const double tab[], size_t n, int x_start, double y_scale, char mark) {
    ...
    for (int i = x_start; i < n+x_start; ++i) {
        ...
        printf("%d |",i);
        int steps = tab[k]/y_scale;
        if (tab[k] - steps*y_scale > 0.5*y_scale)
            steps += 1;
...
void monty_hall(int *p_switch_wins, int *p_nonswitch_wins, int n) {
    *p_switch_wins = 0;
    for (int i = 0; i < n; ++i) {
        int door = rand_from_interval(1,3);
        int player = rand_from_interval(1,3);
        if (door == player){
            *p_switch_wins += 1;
        }
    }
    *p_nonswitch_wins = n-*p_switch_wins;
}
